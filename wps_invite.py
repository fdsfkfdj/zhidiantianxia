...
data = {
    "wps_invite": [
        {
            "name": "臭臭酱",
            "invite_userid": WPS_ID,  # "*********复制手机WPS个人信息中的用户ID进来，类似括号内容(123456789)*************(不保留双引号)",
...
def pushMail(desp, nowtime):
    ...
    if '失败' in desp:
        yag.send(to = [MAIL2], subject = 'WPS小程序邀请结果：', contents = ['亲爱的臭臭酱，本次邀请失败啦！原因是：{}。详情请打开WPS微信小程序查看哦！'.format(desp)])
    else:
        yag.send(to = [MAIL2], subject = 'WPS小程序邀请结果：', contents = ['亲爱的臭臭酱，本次邀请成功啦！本次共邀请了{}人。详情请打开WPS微信小程序查看哦！'.format(PCOUNT)]) #本次共邀请了{}人。详情请打开WPS微信小程序查看哦！'.format(PCOUNT)
